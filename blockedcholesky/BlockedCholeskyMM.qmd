...
We demonstrate this with the `insteval` dataset in the supplemental materials where we show that fitting the model with the order of these two blocks swapped is almost twenty times slower.
...
We consider a large observational study -- ratings of movies by users of [movielens.org](https://movielens.org) made available at the [grouplens.org download site](https://grouplens.org/datasets/movielens) [@Harper2016] as *MovieLens 32M Dataset* (`ml-32m`), consisting of roughly 32 million ratings of over 80,000 movies by over 200,000 users.
...
```{julia}
#| echo: false
#| tbl-cap: Summary of results for the `ml-32m` data modeling run speeds.  The `mc` column is the movie cutoff, `uc` is the user cutoff, `nratings`, `nusers` and `nmvie` are, respectively, the number of observations, users and movies in the reduced data set, `modelsz` and `L22sz` are the model size and `L[2,2]` size (in GiB), `nv` is the number of objective function evaluations until convergence, `fittime` and `evtime` are the number of seconds required to fit the model or to perform a single objective function evaluation.
#| label: tbl-sizespeed
sizespeed = CSV.read("./data/sizespeed.csv", DataFrame; downcast=true)
...
#| fig-cap: "Number of ratings (left) and movies (right) in reduced table by movie cutoff and by user cutoff.  There are three lines in the right panel but they are close to being superimposed."
#| warning: false
...
The time for one evaluation is given in the `evtime` column of @tbl-sizespeed.
...
That is, we can't expect to reproduce `nv` exactly when fitting the same model on different computers or with slightly different versions of software; but we can expect the pattern in `evtime` with respect to the user and movie cutoffs to be reproducible.
...
We benchmarked fitting the same model as in @sec-structure using the {{< pkg lme4 >}} and {{< pkg glmmTMB >}} packages for {{< proglang R >}}.

```
{{< include scripts/benchmarkoutput.txt >}}
```

As of writing, an attempt to fit the `ml-32m` example using {{< pkg glmmTMB >}} has been running for more than a week on the machine that was used for the {{< pkg MixedModels.jl >}} timings and has not yet completed.
Given otherwise similar performance, we do not expect {{< pkg lme4 >}} to perform any better than {{< pkg glmmTMB >}}.
...
```{julia}
print(refit!(m2; progress = false))
```

This fit is equivalent to that of `m1`, taking a comparable number of evaluations of the objective before convergence is declared

```{julia}
length.((m1.optsum.fitlog, m2.optsum.fitlog))
```
\noindent
The number of evaluations is not exactly the same because changes in the order of arithmetic operations during the Cholesky factorization can produce slightly different evaluations of the objective, resulting in slightly different paths to convergence.
For example, the first evaluations of the objective for each model, at the parameter vector $\mbftheta=[1.,1.,1.,1.]$, are

```{julia}
(m1.optsum.finitial, m2.optsum.finitial)
```

The difference in fitting times is dominated by the difference in the time to evaluate the objective

```{julia}
@b objective!($m2, $(m2.optsum.final))
```
\noindent
which is an order of magnitude slower than evaluating the objective for the original model `m1`

```{julia}
@b objective!($m1, $(m1.optsum.final))
```
