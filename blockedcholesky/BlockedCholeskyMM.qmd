...
For the purposes of evaluating the gradient of the profiled log-likelihood or the profiled REML criterion we must only consider the part of the objective that is a linear combination of the logarithms of the diagonal elements of $\mbfL_{\mbftheta}$, which can be written as
$$
...
Premultiplying @eq-Ldot by $\mbfL^{-\top}$ and postmultiplying by $\mbfL^{-1}$ produces
$$
\begin{split}
  \mbfL^{-\top}\dot{\mbfOmega}\mbfL^{-1} & = \mbfL^{-\top}\dot{\mbfL} + \dot{\mbfL}^\top\mbfL^{-1} \\
  & = \mbfL^{-\top}\dot{\mbfL} + \left(\mbfL^{-\top}\dot{\mbfL}\right)^\top
\end{split}
$$ {#eq-Ldotinv}

We note that the $j$th diagonal element of $\mbfL^{-1}\dot{\mbfOmega}\mbfL^{-\top}$ is exactly the partial derivative of $2\log\ell_{jj}$.
Thus the terms in the gradient of the profiled objective are
$$
\frac{\partial f}{\partial\theta_i}=\tr\left(\mbfW\mbfL^{-1}\frac{\partial\mbfOmega}{\partial\theta_i}\mbfL^{-\top}\right)=\tr\left(\mbfL^{-1}\mbfW\mbfL^{-\top}\frac{\partial\mbfOmega}{\partial\theta_i}\right)\quad i=1,\dots,r 
$$ {#eq-traceeval}
where $r$ is the dimension of $\mbftheta$ and $\mbfW$ is a diagonal matrix of the weights in $f$.

In the final expression of @eq-traceeval, which was suggested by Antropic's Claude-fable AI, the evaluation of $\mbfL^{-1}\mbfW\mbfL^{-\top}$ only needs to be evaluated once at a given value of $\mbftheta$.
