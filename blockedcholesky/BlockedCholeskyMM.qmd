...
There is a long history of computational tools for fitting mixed effects models, including the `PROC MIXED` procedure in {{< proglang SAS >}}, and the {{< pkg nlme >}}, {{< pkg lme4 >}} and more recently {{< pkg glmmTMB >}} packages in the {{< proglang R >}} ecosystem.
...
```{julia}
#| output: false
using AlgebraOfGraphics, CairoMakie, PrettyTables, Printf, SparseArrays
...
pretty_table(
  sumry;
  formatters = [fmt__round(2, [4]), fmt_nothing],
...
The [RectangularFullPacked.jl]{.pkg} package allows access to this format in {{< proglang Julia >}}.
...
We demonstrated how the {{< pkg MixedModels.jl >}} package can be used to fit models with two distinct grouping factors (the `ml-32m` example) and three distinct grouping factors (`insteval` example).
...
pretty_table(
  sizespeed;
  formatters = [fmt__round(2, [6, 7, 9, 10])],
  backend = :markdown,
  # show_subheader = false,
)
```
