...
dat.service = (dat.service .== "Y")         # convert to Boolean values
show(
...
The optimization of the profiled objective required about 175 evaluations of the objective function and took about a second on a laptop computer, as shown in this benchmark using the `@be` macro from the {{< pkg Chairmarks.jl >}} package.
...
As described in @sec-one-one-block, the diagonal or block-diagonal structure of the $[1,1]$ of $\mbfA$ carries over to the $[1,1]$ block of $\mbfL$.
...
Allocating a $q_i\times q_i$ array for the i'th diagonal block but only using the lower triangle is fine for small $q_i$ but becomes expensive when the number of levels of the i'th grouping factor is large.

The rectangular full-packed (RFP) storage format [@lawn199], designated `TrRFP` in @tbl-blk_descrip, allows for a triangular matrix to be packed into (roughly) half the amount of storage required for the dense square matrix.
...
The details are not as important as the fact that 3/4 of the original triangular array (the trapezoidal part) is stored essentially the same way as it would be in full triangular storage and only 1/4 of the original array requires customized access methods.
...
If we order these levels by decreasing incidence then usually a substantial majority of the updates occur in the trapezoidal part, with Cartesian or linear indexing, as opposed to the triangular part with a more complicated indexing scheme.

This re-ordering of the levels of the second grouping factor is a heuristic and, in our experience, a successful heuristic.
...
```{julia}
#| echo: false
#| fig-cap: Proportion of memory footprint of the model in L[2,2] versus the overall model size (GiB).  The dots are at different movie cutoff (mc) values, with the largest memory footprints at mc=1 and the smallest at mc=50.
#| label: fig-l22prop
#| warning: false
draw(
