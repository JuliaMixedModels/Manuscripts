...
abstract: |
  @bates.maechler.etal:2015 described the evaluation of the profiled log-likelihood of a linear mixed-effects model by updating a sparse, symmetric positive-definite matrix and computing its Cholesky factor, as implemented in the {{< pkg lme4 >}} package for {{< proglang R >}}.
  Here we present enhancements to the derivation and theoretical presentation of the result and to its implementation using a blocked Cholesky factorization in the {{< pkg MixedModels.jl >}} package for {{< proglang Julia >}} [@Bezanson2017].
  The gain in computational efficiency is primarily due to three factors:
  (1) the new derivation allows us to compute the penalized sum of squared residuals without computing the conditional estimates of the fixed-effects parameters and the conditional modes of the random effects at each optimization step,
  ...
  (3) the multiple dispatch feature of the {{< proglang Julia >}} language allows us to use specialized algorithms for different kinds of matrices instead of relying on generic algorithms during the Cholesky factorization.
...
There is a long history of computational tools for fitting mixed effects models, including the {{< pkg MIXED>}} procedure in {{< proglang SAS >}}, and the {{< pkg nlme >}}, {{< pkg lme4 >}} and more recently {{< pkg glmmTMB >}} packages in the {{< proglang R >}} ecosystem.
The extensive use of these models is shown by the fact that @bates.maechler.etal:2015, which describes the methodology and use of {{< pkg lme4 >}}, is among the top 25 most-cited papers of the twenty-first century [@Pearson2025].
We present {{< pkg MixedModels.jl >}} a new implementation of mixed effects models in the {{< proglang Julia >}} programming language.
This implementation relies on two innovations, one analytical and the other computational.
At the heart of {{< pkg MixedModels.jl >}}, there is a new way of formulating the profiled log-likelihood evaluation that expresses this objective without requiring explicit evaluation of the conditional parameter estimates.
On the computational level, the evaluation of the objective requires the Cholesky factor of a large, sparse, positive-definite symmetric matrix, as in {{< pkg lme4 >}}.
This matrix has a blocked structure that is explicitly exploited in {{< pkg MixedModels.jl >}} but not in {{< pkg lme4 >}}.
...
We exploit the multiple dispatch feature of {{< proglang Julia >}} to do exactly that -- use sparse matrix algorithms for the sparse blocks, and level 3 BLAS and LAPACK subroutines for the dense blocks.
...
However, the implementation of the profiled log-likelihood evaluation in the {{< pkg lme4 >}} package for {{< proglang R >}} is not explicitly based upon the blocks --- {{< pkg lme4 >}} uses a more general sparse Cholesky factorization as implemented in the {{< pkg CHOLMOD >}} library [@10.1145/1391989.1391995].

An important part in a general sparse Cholesky factorization, such as implemented in {{< pkg CHOLMOD >}}, is determining a *fill-reducing permutation* of the rows and columns of the symmetric, positive-definite matrix to be factored.
The main difference between the methods in {{< pkg MixedModels.jl >}} and those in {{< pkg lme4 >}} is that in {{< pkg MixedModels.jl >}} it is the blocks that are permuted, if necessary, rather than individual rows and columns.
In other words, the particular structure of the blocks in the Gram matrix is exploited in {{< pkg MixedModels.jl >}} rather than relying upon general sparse Cholesky techniques that are not able to exploit this structure.
...
{{< proglang Julia >}} [@Bezanson2017] is an ideal environment for this type of calculation because of its efficient implementation of multiple dispatch, its just-in-time compilation, and its rich type system.
...
In @sec-structure we use an example of a fitted mixed-effects model to illustrate our implementation of the blocked Cholesky factorization in {{< pkg MixedModels.jl >}}.
...
The types of linear mixed-effects models that can be fit by {{< pkg lme4 >}} or {{< pkg MixedModels.jl >}} are based on two vector-valued random variables: the $q$-dimensional vector of random effects, $\mcB$, and the $n$-dimensional response vector, $\mcY$.
...
The computational methods in {{< pkg lme4 >}} and in {{< pkg MixedModels.jl >}} are based on $\mbfLambda_\theta$ and do not require evaluation of $\mbfSigma_\theta$.
...
\noindent
which is an affine function of the logarithms of the diagonal elements of $\mbfR$.
...
The two values needed for the log-likelihood evaluation, $2\log(|\mbfR_{ZZ}|)$ and $r_{yy}^2$, are obtained directly from the diagonal elements of the Cholesky factor.
...
In {{< pkg MixedModels.jl >}} the `LinearMixedModel` struct contains a blocked representation of $\mathbf{A}$ in the `A` field and a similarly structured lower-triangular blocked array in the `L` field.
...
The user interface in {{< pkg MixedModels.jl >}} is intentionally similar to that of {{< pkg lme4 >}}.
...
(In {{< proglang Julia >}} a formula expression such as this must be enclosed in a call to the `@formula` macro.)
...
For convenience, Table 3 from  @bates.maechler.etal:2015 has been reproduced with slight changes as @tbl-re-sizes.
...
The blocked derivation in @bates.maechler.etal:2015 carries over into the model representation in {{< pkg MixedModels.jl >}} with one modification --- in {{< pkg lme4 >}} the (implicit) partitioning of $\mbfZ$ into $k$ vertical blocks is according to the random-effects terms in the formula, whereas in {{< pkg MixedModels.jl >}} it is according to the grouping factors.
...
The `InstEval` dataset from the {{< pkg lme4 >}} package is reproduced as `insteval` in {{< pkg MixedModels.jl >}}.
(The {{< proglang Julia >}} convention is to use lower-case for names of objects and camel-case for names of types and for package names.)
...
Further details are given on the {{< proglang R >}} help page for `lme4::InstEval`.

In {{< proglang Julia >}} we attach the packages to be used, load the data set, convert it to a `DataFrame` and evaluate some descriptive statistics on its columns to produce @tbl-insteval-summary,
...
using AlgebraOfGraphics, CairoMakie, Printf, SparseArrays
using Arrow, Chairmarks, CSV, DataFrames, LinearAlgebra, MixedModels
dat = DataFrame(MixedModels.dataset("insteval"))
dat.service = (dat.service .== "Y")         # convert service to a Boolean vector
show(
...
The model formula is chosen to illustrate certain aspects of the conversion from a model formula to the internal model representation.
...
m1 = fit(MixedModel, form, dat, progress=false) # suppress display of progress bar
print(m1)
```
...
The optimization of the profiled objective required about 175 evaluations of the objective function and took about one and a half seconds on a laptop computer, as shown in this benchmark using the `@be` macro from the {{< pkg Chairmarks.jl >}} package.

```{julia}
@be fit($MixedModel, $form, $dat; progress=false)  seconds=8
```

The environment in which these benchmarks were performed is described in the supplemental materials.
...
In Julia hot-swapping BLAS implementations is as easy as loading the appropriate package, such as {{< pkg AppleAccelerate.jl >}} or {{< pkg MKL.jl >}}.
...
```{julia}
@be objective!($m1, $(m1.optsum.final))
```
...
```
\noindent
In the notation of @bates.maechler.etal:2015[Section 2] the second parameter of each of these types is $p_i, i = 1,2,3$.
...
```
\noindent
In the notation of @bates.maechler.etal:2015[Section 2] these are $\ell_i, i=1,2,3$.

Finally the numbers of columns in each of the $\mbfZ_i, i=1,2,3$ are
...
The blocks of the augmented Gram matrix, $\mbfA$, and the lower Cholesky factor, $\mbfL_\theta=\mbfR^{\top}_\theta$, are summarized in @tbl-blk_descrip

```{julia}
#| tbl-cap: Block Structure of the `A` and `L` matrices in the `insteval` example
#| label: tbl-blk_descrip
...
In the block structure summary (@tbl-blk_descrip) if a block of $\mbfL$ and its corresponding block of $\mbfA$ have the same structure, the name of that structure is given once.
This is the case for all of the blocks except the [2,2] block, which is diagonal in $\mbfA$ but a dense lower triangular matrix, stored in the Rectangular Full Packed format [@lawn199] in $\mbfL$, the [2,3] block, which is sparse in $\mbfA$ but dense in $\mbfL$, and the [3,3] block, which is block-diagonal in $\mbfA$ and triangular dense (full storage) in $\mbfL$.
...
The last diagonal block, corresponding to the fixed-effects and response, is the $(p+1)\times(p+1)$ identity matrix.
...
## The Rectangular Full Packed (RFP) format {#sec-RFP}

The diagonal blocks of $\mbfA$ generated by random-effects terms are either diagonal, for scalar random effects, or block diagonal, consisting of $\ell_i$ small blocks of size $p_i\times p_i$, for vector-valued random effects.
As described in @sec-one-one-block, the diagonal or block-diagonal structure of the $[1,1]$ block of $\mbfA$ carries over to the $[1,1]$ block of $\mbfL$.
However, with more than one grouping factor in a non-nested design, the diagonal blocks of $\mbfL$ below the $[1,1]$ block get ``filled-in" with non-zeros in most or all of the lower triangle.
Allocating a $q_i\times q_i$ array for the i'th diagonal block of $\mbfL$ but only using the lower triangle is fine for small $q_i$ but becomes expensive when the number of levels of the i'th grouping factor is large.

The rectangular full-packed (RFP) storage format [@lawn199] for triangular or symmetric/Hermitian matrices, designated `TrRFP` in @tbl-blk_descrip, allows for a triangular matrix to be packed into (roughly) half the amount of storage required for the dense square matrix.
For the $[2,2]$ block in @tbl-blk_descrip, a dense square matrix of size $1128\times 1128$ requires about 9.71 MB of storage whereas the RFP format, which is stored in a rectangular array of size $1129\times 564$ requires about 4.86 MB.

The {{< pkg RectangularFullPacked.jl >}} package allows access to this format in {{< proglang Julia >}}.

Full expanation of all forms of RFP storage requires considerable detail, for which see @lawn199, but the broad outline for a lower triangular matrix is that the first $n/2$ (rounded up for odd $n$) columns are stored unchanged in the rectangular backing array.
For an even $n$ the number of rows in the backing array is $n + 1$ and the columns of this "trapezoidal" part of the original array are shifted down by one row.
The triangular part from the lower right of the original array is transposed and stored in the upper right corner of the backing array.

The details are not as important as the fact that 3/4 of the original triangular array (the trapezoidal part from the first $n/2$ columns) is stored essentially the same way as it would be in full triangular storage and only 1/4 of the original array requires customized access methods.
...
It is described as a "symmetric rank-k update" and involves replacing the current $\mbfL[2,2]$ by $\mbfL[2,2]-\mbfL[2,1]\mbfL[2,1]^\top$ (because of the negative sign in that expression this is actually a "downdate").
...
The number of updates done to the triangular part of the RFP representation of $\mbfL[2,2]$ versus the trapezoidal part depends on ordering of the levels of the second grouping factor.
If we order these levels by decreasing incidence then usually a substantial majority of the updates occur in the trapezoidal part, using Cartesian or linear indexing, as opposed to the triangular part with a more complicated indexing scheme.

This re-ordering of the levels of the second grouping factor is a heuristic but, in our experience, a successful heuristic for reducing the time to perform the downdate operation.

Two arguments to the `fit` method for a linear mixed model, `RFPthreshold`, which defaults to 1000, and `sortlevels`, which defaults to `true`, control whether RFP storage will be used for a dense triangular block of $\mbfL$ and whether the levels of the corresponding grouping factor will be sorted by descreasing incidence.

We can fit the `insteval` model with triangular storage for $\mbfL[2,2]$ as

```{julia}
#| tbl-cap: Block Structure of the `A` and `L` matrices in the full storage `insteval` example
#| label: tbl-blk_descrip_f
#| tbl-cap-location: top
m1f = fit(MixedModel, form, dat; RFPthreshold=1500, progress=false);
BlockDescription(m1f)
```
\noindent
for which evaluation of the objective takes

```{julia}
@be objective!($m1f, $(m1f.optsum.final))
``` 

That is, evaluation of the objective is slightly faster with the full square storage of $\mbfL[2,2]$, compared to the RFP storage, but at the expense of more memory usage

```{julia}
(Base.summarysize(m1), Base.summarysize(m1f))
```
...
second(x) = x[2]   # why not just use `last`?
...
(except in the case that the levels of the first grouping factor are strictly nested within the levels of the second grouping factor).
...
In practice this change gives an order of magnitude difference between the runtimes for fitting the two versions of the model for the `insteval` data.
...
The supplemental materials contain a {{< proglang Julia >}} script, `ml32-download.jl` to perform the downloading and extraction of the csv file and creation of the subsequent tables. 
...
```{julia}
#| echo: false
#| fig-cap: "Empirical cumulative distribution plots of the number of ratings per movie and per user.  The horizontal axes are on a logarithmic scale."
...
These fits were performed on a cloud instance with a generous amount of memory (128 GiB) and using 12 threads and the Intel Math Kernel Library (MKL) for numerical linear algebra.
The results are summarized in @tbl-sizespeed.
...
```{julia}
#| echo: false
#| tbl-cap: Summary of results for the `ml-32m` data modeling run speeds.  The `mc` column is the movie cutoff, `uc` is the user cutoff, `nratings`, `nusers` and `nmvie` are, respectively, the number of observations, users and movies in the reduced data set, `modelsz` and `L22sz` are the model size and `L[2,2]` size (in GiB), `nv` is the number of objective function evaluations until convergence, `fittime` and `mnevtm` are the number of seconds required to fit the model and the mean time to perform a single objective function evaluation.
#| label: tbl-sizespeed
sizespeed = CSV.read("./data/sizespeed.csv", DataFrame; downcast=true)
show(sizespeed; row_labels=nothing, eltypes=false, allrows=true, summary=false)
```
...
As shown in the first panel of @fig-nratingsbycutoff, the number of ratings varies from a little over 27 million to 32 million.
...
#| fig-cap: "Number of ratings (left) and movies (right) in reduced table by movie cutoff and by user cutoff.  There are three lines in the right panel but they are essentially superimposed."
#| warning: false
...
(There are three different, but nearly coincident, lines in this panel.)

### Memory footprint of the model representation {#sec-lrgobsmemprint}

@fig-memoryfootprint shows that the overall memory size (the "memory footprint") of the model representation depends primarily on the movie cutoff.
(There are also three distinct, but nearly coincident, lines in @fig-memoryfootprint, corresponding to the three user cutoff values.)
...
@fig-l22prop shows the dominance of the `L[2,2]` block, which is a dense lower triangular matrix (in RFP format), in the overall memory footprint of the model.
When all the movies are included in the data to which the model is fit the total memory footprint is over 30 GiB, and over 80% of that memory is for `L[2,2]`.
Even when requiring a minimum of 80 ratings per movie, `L[2,2]` accounts for about 35% of the memory footprint.

```{julia}
#| echo: false
#| fig-cap: Proportion of memory footprint of the model in L[2,2] versus the overall model size (GiB).  The dots are at different movie cutoff (mc) values, with the largest memory footprints at mc=2 and the smallest at mc=50.
#| label: fig-l22prop
#| warning: false
draw(
...
Note that although the dimensions of the `L[2,1]` block are larger than those of the `L[2,2]` block its memory footprint is smaller than that of the `L[2,2]` block, because it is stored as a sparse matrix.
...
Also, the number and positions of the non-zeros in `L[2,1]` is exactly the same as those in `A[2,1]`.
That is, there is no fill-in in the `[2,1]` block

The fact that the memory footprint of the model is dominated by that of the `[2,2]` block makes the use of the RFP format for that block effective in reducing memory usage.
...
### Speed of log-likelihood evaluation {#sec-loglikelihoodeval}
...
The mean time per evaluation is given in the `mnevtm` column of @tbl-sizespeed.
Also given is the number of evaluations to convergence, `nv`, and the total time to fit the model, `fittime`.
The reason for considering `mnevtm` in addition to `fittime` and `nv` is because the `mnevtm` for one model, relative to other models, is reasonably stable across computers whereas `nv`, and hence, `fittime`, can be affected by seemingly trivial variations in function values resulting from different implementations of low-level calculations, such as the BLAS (Basic Linear Algebra Subroutines).
...
That is, we can't expect to reproduce `nv` exactly when fitting the same model to the same data but on different computers or with slightly different versions of software.
However, we can expect the pattern in `mnevtm` with respect to the user and movie cutoffs to be reproducible.

As shown in @fig-mnevtmvsl22 the evaluation time for the objective is predominantly a function of the size of the `[2,2]` block of `L`.

```{julia}
#| echo: false
#| fig-cap: "Evaluation time for the objective (s) versus size of the [2,2] block of L (GiB)"
#| label: fig-mnevtmvsl22
#| warning: false
draw(
  data(sizespeed) *
  mapping(
    :L22sz => "Size of [2,2] block of L (GiB)",
    :mnevtm => "Time for one evaluation of objective (s)",
    color = :uc => renamer([20 => "≥20 ratings", 40 => "≥40 ratings", 80 => "≥80 ratings"])  => "per user",) *
    visual(ScatterLines);
    legend = (; position = :bottom, titleposition = :left),
)
...
# Further Comments and directions for research {#sec-further-comments}

## REML estimation {#sec-REML}
...
For instance:
```{julia}
mreml = fit(MixedModel, form, dat; REML=true, progress=false);
print(mreml)
```
\noindent
The estimates obtained above are the same as those obtained from `lme4::lmer` function in {{< proglang R >}}.

We used the formula `y ~ 1 + service + (1 | d) + (1 | s) + (1 | dept) + (0 + service | dept)` to demonstrate that {{< pkg MixedModels.jl >}} amalgamates random effects terms with the same grouping factor.
In practice we can use the more concise formula: `y ~ 1 + service + (1 | d) + (1 | s) + zerocorr(service | dept)`.
That is, the term `zerocorr(service | dept)` is equivalent to adding the two terms `(1 | dept) + (0 + service | dept)`, and the same as the {{< pkg lme4 >}} formula term `(1 + service || dept)`.

Although we only demonstrated the fitting of linear mixed models, {{< pkg MixedModels.jl >}} can also fit generalized linear mixed models including logistic, probit, poisson, and negative binomial models.
...
## Evaluation of the gradient of the profiled objective

As mentioned in @sec-profiled, the profiled log-likelihood is an affine function of the logarithms of the diagonal elements of the blocked Cholesky factor, $\mbfL_{\mbftheta}$.
The detailed derivation of the profiled REML criterion in @bates.maechler.etal:2015 shows that it too is an affine function of the logarithms of the diagonal elements of $\mbfL_{\mbftheta}$.
For the purposes of evaluating the gradient of the profiled log-likelihood or the profiled REML criterion we only must consider the part of the objective that is a linear combination of the logarithms of the diagonal elements of $\mbfL_{\mbftheta}$, which can be written as
$$
f(\mbftheta)=\sum_{j=1}^{q+p+1}w_j(2\log\ell_{jj}) ,
$$ {#eq-linearcomb}
\noindent
where the weights, $w_j, j=1,\dots,q+p+1$ for the profiled log-likelihood are $1$ for $j=1,\dots,q$ (the random-effects rows), $0$ for $j=q+1,\dots,q+p$ (the fixed-effects rows), and $n$ for $j=q+p+1$ (the response row).
As shown in @bates.maechler.etal:2015 the weights are $1$ for $j=1,\dots,(q+p)$ (both random- and fixed-effects rows) and $n-p$ for $j=q+p+1$ for the profiled REML criterion.

### Symbolic differentiation of the Cholesky factor

Following @murray2016, we use symbolic differentiation of the defining relationship
$$
\mbfOmega_\mbftheta=\mbfL_\mbftheta \mbfL_\mbftheta^\top
$$ {#eq-Omegadef}
\noindent
to produce
$$
\dot{\mbfOmega}=\dot{\mbfL}\mbfL^\top+\mbfL\dot\mbfL^\top
$$ {#eq-Ldot}
\noindent
where the dot indicates partial differentiation with respect to an element of $\mbftheta$
(in this and subsequent equations we drop the subscript $\mbftheta$ on $\mbfOmega$ and $\mbfL$).

Premultiplying @eq-Ldot by $\mbfL^{-1}$ and postmultiplying by $\mbfL^{-\top}$ produces
$$
\mbfL^{-1}\dot{\mbfOmega}\mbfL^{-\top} = \mbfL^{-1}\dot{\mbfL} + \dot{\mbfL}^\top\mbfL^{-\top}
= \mbfL^{-1}\dot{\mbfL} + \left(\mbfL^{-1}\dot{\mbfL}\right)^\top\,.
$$ {#eq-Ldotinv}
\noindent
Because $\mbfL$, $\dot{\mbfL}$, and $\mbfL^{-1}$ are all lower triangular, $\mbfL^{-1}\dot{\mbfL}$ is also lower triangular and the $j$th diagonal element of $\mbfL^{-1}\dot{\mbfOmega}\mbfL^{-\top}$ is exactly the partial derivative of $2\log\ell_{jj}$.
Thus the terms in the gradient of the profiled objective are
$$
\frac{\partial f}{\partial\theta_i}=
\tr\left(\mbfW\mbfL^{-1}\frac{\partial\mbfOmega}{\partial\theta_i}\mbfL^{-\top}\right)=
\tr\left(\mbfL^{-\top}\mbfW\mbfL^{-1}\frac{\partial\mbfOmega}{\partial\theta_i}\right)\quad i=1,\dots,r 
$$ {#eq-traceeval}
where $r$ is the dimension of $\mbftheta$ and $\mbfW$ is a diagonal matrix of the weights in $f$.

In the final expression of @eq-traceeval, which was suggested by Antropic's Claude-fable AI, the symmetric matrix $\mbfL^{-\top}\mbfW\mbfL^{-1}$ only needs to be evaluated once for a given value of $\mbftheta$.
That is, it does not depend upon $i=1,\dots,r$.
Thus the cost of evaluating this expression can be amortized over all the elements of the gradient.

Preliminary investigations show that gradient evaluations are most effective in reducing the effort to optimize the objective when the dimension of $\mbftheta$ is moderate to large.
...
We demonstrated how the {{< pkg MixedModels.jl >}} package can be used to fit models with two distinct grouping factors (the `ml-32m` example) and three distinct grouping factors (the `insteval` example).
Fitting models with a single grouping factor or with more than three grouping factors is straightforward.

The key contribution of {{< pkg MixedModels.jl >}} is the new fitting procedure based on the augmented matrix $\mbfA$ (@eq-A) and the simplified formula for the profiled loglikelihood (@eq-profiled-log-likelihood).
...
This update is performed in-place in the storage for $\mbfL$.
...
Finally, the multiple dispatch feature of the {{< proglang Julia >}} language allows us to use specialized algorithms for different kinds of matrices instead of relying on generic algorithms.
...
All figures were produced using the {{< pkg AlgebraOfGraphics.jl >}} package with the {{< pkg CairoMakie.jl >}} backend in the {{< pkg Makie.jl >}} graphics system [@DanischKrumbiegel2021].
Julius Krumbiegel and Anshul Singhvi provided invaluable help and feedback with {{< pkg Makie.jl >}}.
The plots in this work would not have been possible without them.
Finally, we would like to thank the {{< proglang Julia >}} community for bugfixes and answering our questions in the community support channels.
