...
In the supplemental materials we show that this change gives an order of magnitude difference between the runtimes for fitting the two versions of the model for the `insteval` data.
...
That is, we can't expect to reproduce `nv` exactly when fitting the same model on different computers or with slightly different versions of software; but we can expect the pattern in `mnevtm` with respect to the user and movie cutoffs to be reproducible.
